...
const DriftWall = ({
  ...
  const renderTile = (item, id, colIndex) => {
    ...
    if (item.href) {
      return (
        <a key={id} href={item.href} target="_blank" rel="noreferrer noopener" suppressHydrationWarning {...commonProps}>
          {inner}
        </a>
      );
    }
    return (
      <div key={id} tabIndex={0} role="button" aria-label={item.title ?? 'tile'} suppressHydrationWarning {...commonProps}>
        {inner}
      </div>
    );
  };
